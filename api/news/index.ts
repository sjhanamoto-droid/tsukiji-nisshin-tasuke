...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    ...
    return res.status(200).json({ items: rows.map(rowToNews) });
  }
  ...
  if (req.method === 'POST') {
    ...
    const rows = await sql`
      ...
      VALUES (${id}, ${title}, ${category}, ${date}, ${coverImageUrl}, ${contentHtml}, ${JSON.stringify(images)}::jsonb)
      RETURNING *`;
    return res.status(201).json({ item: rowToNews(rows[0]) });
  }
